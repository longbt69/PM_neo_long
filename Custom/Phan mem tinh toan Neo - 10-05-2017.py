...
class KetQuaTinh_NguyenLy_0(bpy.types.Operator):
	...
	def execute(self, context):
		...
		if nguyenLy[0] == '1':
			...
			T = ((a * Y * A * R) / (k * (1 - c / (Y * R)) * tan(g_m_s_t))) * ((1 - (math.e) ** (-tan(g_m_s_t) * k * D / R)) / (1 - (math.e) ** (tan(g_m_s_t) * k * L / R))) + 700
			# print(T)
...
class RegisterParameter(bpy.types.PropertyGroup):
	...
	ban_kinh_chiu_cat_cua_dv_dat_da_duoc_gia_co_NL2 		= 0.5	# 5		Bán kính chịu cắt của Đơn vị Đất Đá được gia cố							4.0

	# -DATDA
	trong_luong_rieng_của_dam_da_mang_tai_Y_NL2 			= 2.5	# 6		Trọng lượng riêng của dầm đá mang tải									2.7
	goc_ma_sat_trong_NL2 									= 35.0	# 7		Góc ma sát trong														35.0
	ty_so_giua_ap_luc_ngang_tb_va_thang_dung_tb_k_NL2 		= 0.5	# 8		Tỷ số giữa áp lục ngang trung bình và áp lực thằng đứng trung bình		1.0
	luc_ket_dinh_giua_cac_lop_dat_da_c_NL2 					= 0.0	# 9		Lực kết dính giữa các lớp đất đá										10.0
